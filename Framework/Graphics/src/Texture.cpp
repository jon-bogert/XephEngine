...
{
	ASSERT(m_shaderResourceView == nullptr, "Texture: terminate must be called before destruction");
}
...
void xe::Graphics::Texture::Initialize(const std::filesystem::path& fileName)
{
	...
	HRESULT hResult = DirectX::CreateWICTextureFromFile(device, context, fileName.c_str(), nullptr, &m_shaderResourceView);
